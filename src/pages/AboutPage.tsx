...
export function AboutPage({ projectUrl, onOpenLink }: { projectUrl: string; onOpenLink: (url: string) => void }) {
  ...
  return <div className="page about-page">
    ...
    <section className="about-grid">
      ...
      <article className="panel about-project"><h2>Community project</h2><p>ZCOM Mod Manager is an independent community project and is not affiliated with or endorsed by Lucasfilm Games, Electronic Arts, Bit Reactor, or Respawn Entertainment.</p><div className="settings-actions"><button onClick={() => onOpenLink(projectUrl)}><Github aria-hidden size={17} />View source on GitHub</button><button onClick={() => onOpenLink(`${projectUrl}/releases`)}><ExternalLink aria-hidden size={17} />All releases</button></div><small>Licensed under the MIT License · © 2026 ZCOM Mod Manager contributors</small></article>
    </section>
  </div>;
}
